...
public class MeteorEvent : MonoBehaviour, IGameEvent
{
    ...
    [Tooltip("Radio de da�o en �rea alrededor del piunto de impacto")] 
    ...
    [Tooltip("Prefab del meteorito (opcional, para animaci�n de ca�da)")]
    public GameObject meteorPreFab;

    [Tooltip("Prefab del indicador de advertencia en el suelo (opcional)")]
    public GameObject warningIndicatorPreFab;

    [Tooltip("Prefab del efecto de explosi�n al impactar (opcional)")]
    ...
    private void ApplyAreaDamage (Vector2 center)
    {
        ...
        if (hits.Length == 0)
        {
            Debug.Log("No hay enemigos en el �rea");
            return;
        }
        ...
        foreach (Collider2D hit in hits)
        {
          ...
          if (enemyHealth != null )
          {
                enemyHealth.TakeDamage(impactDamage);
                  Debug.Log($"[MeteorEvent] Da�o a {hit.gameObject.name}: {impactDamage}");
        }
        }
